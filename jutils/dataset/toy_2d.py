...
def sample_spirals(n, noise=0.5):
    n = n // 2  # 2 arms
...
def sample_gaussianmixture(n, loc1=(-3, -3), loc2=(3, 3), var1=0.5, var2=1.0, p1=0.2, p2=0.8):
...
def sample_mixture_of_gmm(n, cfg, class_idx: int = None):
    ...
    """
    assert len(cfg["class_probs"]) == len(
        cfg["classes"]
    ...
    if class_idx is not None:
        ...
        classes_counts = torch.multinomial(torch.tensor(cfg["class_probs"]).float(), n, replacement=True).unique(
    ...
    for c_idx, c_count in zip(*classes_counts):
        ...
        for m_idx, m_count in zip(*modes_counts):
            assert len(class_info["mode_probs"]) == len(
                class_info["params"]
...
def sample_2gaussians(n, scale=4.0, var=0.2, dim=2):
    ...
    data = torch.stack([centers[multi[i]] + noise[i] for i in range(n)]).float()
    return data, multi.long()


def sample_5gaussians(n, scale=4.0, var=0.2):
...
def sample_8gaussians(n, scale=5, var=0.1):
    m = torch.distributions.multivariate_normal.MultivariateNormal(torch.zeros(2), math.sqrt(var) * torch.eye(2))
    centers = [
        (1, 0),
        (-1, 0),
        (0, 1),
...
def sample_n_concentric_circles(n, n_circles=3, noise=0.1, scale=0.5):
    ...
    for i, sq_samples in enumerate(samples_per_circle):
        circle_data, _ = datasets.make_circles(n_samples=sq_samples, factor=0.99, noise=noise / (i + 1))
