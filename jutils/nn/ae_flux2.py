...
class FLUX2AutoencoderKL(nn.Module):
    def __init__(self, ckpt_path: str = None, params: AutoEncoderParams = AutoEncoderParams()):
        super().__init__()
        self.params = params
        ...
        if exists(ckpt_path):
            assert os.path.exists(ckpt_path), f"[FLUX.2/AutoencoderKL] Checkpoint {ckpt_path} not found!"
            print(f"[FLUX.2/AutoencoderKL] Loading checkpoint from {ckpt_path}")
            ...
            warnings.warn(f"[FLUX.2/AutoencoderKL] No checkpoint provided. Random initialization.")
    ...
    def decode(self, z: Tensor) -> Tensor:
        z = rearrange(z, "... c (i pi) (j pj) -> ... (c pi pj) i j", pi=self.ps[0], pj=self.ps[1])
